...
// 1. Get accessible team IDs first
if ($role === 'admin') {
  $teamIds = [];
  $allTeams = $conn->query("SELECT team_id FROM teams");
  while ($row = $allTeams->fetch_assoc()) $teamIds[] = $row['team_id'];
} else {
...
if ($teamId && !in_array($teamId, $teamIds)) {
  die("<div class='alert alert-danger m-5'>❌ You are not authorized to manage this team.</div>");
}
